import { Box, Typography, Stack, CardMedia, Paper, List, ListItem, IconButton, Button, useMediaQuery, useTheme } from "@mui/material";
...
export default function Indiv3lb({ FullImageRender }) {
  ...
  return (
    <Box sx={{ paddingRight: '12%', paddingLeft: '12%', paddingTop: isMobile ? '20%' : '10%', paddingBottom: '10%', textAlign: 'left' }}>
      ...
      */}
      <Stack direction={isMD ? "column" : "row"} spacing={isMD ? "10%" : "5%"} width={'100%'} height={'100%'} mt={7} mb={10} justifyContent={"center"} alignItems={isMD ? "center" : "left"} >
        <Stack direction={"column"} spacing={'5%'} height={"80%"} width={isMD ? "100%" : "75%"} >
          ...
          <Box width={'100%'} >
            ...
            <Typography mb={3} mt={2} variant= {isMobile ? "mobileBody" : "desktopBody2"} display="block">{robotInfo.description}</Typography>
          </Box>
          
          {/* Design Section */}
          <Box>
            <Typography variant={isMobile ? "mobileH3" : 'desktopH3'} gutterBottom >Design</Typography>
            <Typography mb={3} mt={2} variant={isMobile ? "mobileBody" : 'desktopBody2'} display="block">{robotInfo.design}</Typography>
          </Box>
